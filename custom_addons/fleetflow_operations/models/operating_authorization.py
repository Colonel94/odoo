...
class FleetflowOperatingAuthorization(models.Model):
    ...
    def action_verify(self):
        if not self.env.user.has_group("fleetflow_operations.group_ops_compliance"):
            raise ValidationError(_("Only a compliance reviewer can verify an authorization."))
        self.write({
            "state": "verified",
    ...
    def action_reject(self):
        if not self.env.user.has_group("fleetflow_operations.group_ops_compliance"):
            raise ValidationError(_("Only a compliance reviewer can reject an authorization."))
        self.write({"state": "rejected", "verified_by": self.env.uid,
                    "verified_on": fields.Datetime.now()})
