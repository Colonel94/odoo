...
**Correction commits:** `77ca3a24` → `9e2e96fd` (six increments, below).
...
- After this correction (`9e2e96fd`): **106** tests, **0 failed, 0 error** — verified after **each** of the six increments (independent full runs A–F), never a single end-run.
- Net new automated tests: **47** operations tests (reproduction-first for F01–F06; regression for F07–F09). All 15 maintenance tests retained and passing.
...
| **F07** | Locking misses holds/evidence/channel changes | **Partial** — protocol Fixed-and-tested; exception process Open | `constants.bump_resource_locks` in hold/credential/channel mutations; `test_concurrency.py` (hardened reservation race asserts serialization loser; new checkout-vs-hold race) · `0f06cf8c` |
...
- **F07:** a **standing exception process** that re-flags active/future allocations after an invalidation committed *after* handover is **Open** (the review lists this as a missing feature, "Live readiness / expiry exceptions"). Dedicated two-connection tests exist for reservation/reservation and checkout/hold; the remaining race pairs (checkout/evidence-revocation, checkout/channel-suspension, reschedule/reservation, checkout/cancel, duplicate checkout/return) are covered by the same lock protocol but do **not** each have a dedicated threaded test yet. Company-wide invalidations (authorization/profile) do not yet bump a company-scope lock. **Open.**
...
```bash
git checkout feat/fleetflow-ops1            # correction increments 77ca3a24..9e2e96fd
python fleetflow/check_static.py            # structural checks
python fleetflow/manage.py test             # 106 tests, 0 failed, 0 error (isolated containers)
```

No merge, push, deployment, paid infrastructure, real customer identity or Odoo
